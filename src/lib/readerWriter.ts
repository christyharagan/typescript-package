...
export function writePackageSchema(pkgDir: string, options?: ts.CompilerOptions, host?: ts.CompilerHost, schemaPath?: string): s.KeyValue<s.Module> {
...
export function readPackageSchema(pkgDir: string, schemaPath?: string): s.KeyValue<s.Module> {
  schemaPath = path.join(pkgDir, schemaPath || 'packageSchema.json')
