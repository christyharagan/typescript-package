...
export function generateRawPackage(pkgDir: string, options?: ts.CompilerOptions, host?: ts.CompilerHost): s.KeyValue<s.RawTypeContainer> {
  let allRawModules: s.KeyValue<s.RawTypeContainer> = {}
...
export function generateRawModules(p: ts.Program, rootDir: string, relativePrefix?: string | utils.RelativePrefix): s.KeyValue<s.RawTypeContainer> {
  let modules: s.KeyValue<s.RawTypeContainer> = {}
  ...
  function processType(type: ts.Type): s.RawType {
    switch (type.flags) {
      ...
      case ts.TypeFlags.Union:
        ...
        return <s.RawUnionOrIntersectionType>{
      ...
      case ts.TypeFlags.Anonymous:
        ...
        switch (declaration.kind) {
          ...
          case ts.SyntaxKind.ObjectLiteralExpression:
            let literalCompositeType: s.RawCompositeType = {
            ...
            ole.properties.forEach(function(property: ts.ObjectLiteralElement) {
              literalCompositeType.members[<string>getName(property.name)] = {
  ...
  function populateProtoClass(clsLikeDec: ts.ClassLikeDeclaration, instanceType: s.RawDecoratedCompositeType, staticType: s.RawDecoratedCompositeType) {
  ...
  function processExpression(expression: ts.Expression): s.RawExpression {
    switch (expression.kind) {
      ...
      case ts.SyntaxKind.ClassExpression:
        ...
        let protoClass: s.RawProtoClass = {
      ...
      case ts.SyntaxKind.FunctionExpression:
        ...
        return <s.RawFunctionExpression>{
      ...
      case ts.SyntaxKind.CallExpression:
        ...
        return <s.RawFunctionCallExpression>{
      ...
      case ts.SyntaxKind.StringLiteral:
        return <s.RawPrimitiveExpression<string>>{
          expressionKind: s.ExpressionKind.PRIMITIVE,
          primitiveTypeKind: s.PrimitiveTypeKind.STRING,
          primitiveValue: (<ts.LiteralExpression>expression).text
        }
      case ts.SyntaxKind.TrueKeyword:
        return <s.RawPrimitiveExpression<boolean>>{
          expressionKind: s.ExpressionKind.PRIMITIVE,
          primitiveTypeKind: s.PrimitiveTypeKind.BOOLEAN,
          primitiveValue: true
        }
      ...
      case ts.SyntaxKind.NumericLiteral:
        return <s.RawPrimitiveExpression<number>>{
          expressionKind: s.ExpressionKind.PRIMITIVE,
          primitiveTypeKind: s.PrimitiveTypeKind.NUMBER,
          primitiveValue: parseFloat((<ts.LiteralExpression>expression).text)
        }
      case ts.SyntaxKind.ObjectLiteralExpression:
...
function isExported(node: ts.Node) {
  return node.modifiers.filter(modifier=> modifier.kind === ts.SyntaxKind.ExportKeyword)
}
